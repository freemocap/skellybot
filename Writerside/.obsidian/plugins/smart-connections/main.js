...
var SMART_TRANSLATION = {
  ...
  es: {
    ...
    initial_message:
      'Hola, soy ChatGPT con acceso a tus apuntes a trav\xE9s de Smart Connections. Hazme una pregunta sobre tus apuntes e intentar\xE9 responderte.',
  },
  fr: {
    pronous: [
      'me',
    ...
    initial_message:
      "Bonjour, je suis ChatGPT et j'ai acc\xE8s \xE0 vos notes via Smart Connections. Posez-moi une question sur vos notes et j'essaierai d'y r\xE9pondre.",
  },
  de: {
    pronous: [
      'mein',
    ...
    initial_message:
      'Hallo, ich bin ChatGPT und habe \xFCber Smart Connections Zugang zu Ihren Notizen. Stellen Sie mir eine Frage zu Ihren Notizen und ich werde versuchen, sie zu beantworten.',
  },
  it: {
    pronous: [
      'mio',
    ...
    initial_message:
      'Ciao, sono ChatGPT e ho accesso ai tuoi appunti tramite Smart Connections. Fatemi una domanda sui vostri appunti e cercher\xF2 di rispondervi.',
  },
};
  ...
  async update_to_v2() {
    if (!this.settings.license_key)
      return new Obsidian.Notice(
        '[Smart Connections] Supporter license key required for early access to V2',
      );
...
`;
  ...
  async get_embeddings_batch(req_batch) {
    ...
    const requestResults =
      await this.request_embedding_from_input(embed_inputs);
    if (!requestResults) {
...
${first_ten_lines[i]}:`;
  ...
  render_external_link_elm(meta) {
    if (meta.source) {
      if (meta.source === 'Gmail') meta.source = '\u{1F4E7} Gmail';
...
var SmartConnectionsSettingsTab = class extends Obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new Obsidian.Setting(containerEl)
      .setName('Get v2')
      .setDesc(
        'Get v2 (warning: very early beta release, likely to crash, please send issues directly to the supporter email for quick response)',
      )
      .addButton((button) =>
        button.setButtonText('Get v2 (unstable)').onClick(async () => {
...
var SmartConnectionsChatView = class extends Obsidian.ItemView {
  ...
  async get_context_for_prompt(nearest) {
    let context = [];
    const MAX_SOURCES =
      this.plugin.settings.smart_chat_model === 'gpt-4-1106-preview' ? 42 : 20;
