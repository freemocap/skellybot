    ...
    new import_obsidian.Setting(containerEl)
      ...
      .addText((text) =>
        text
          ...
          .setValue(
            this.plugin.settings.executeTemplate ||
              DEFAULT_SETTINGS.executeTemplate,
          )
          .onChange((value) => {
            value = value.trim();
...
var OpenVSCode = class extends import_obsidian2.Plugin {
    ...
    this.refreshIconRibbon = () => {
      ...
      if (this.settings.ribbonIcon) {
        this.ribbonIcon = this.addRibbonIcon('vscode-logo', 'VSCode', () => {
          const ribbonCommand = this.settings.ribbonCommandUsesCode
            ? 'openVSCode'
            : 'openVSCodeUrl';
          this[ribbonCommand]();
        });
  ...
  async onload() {
    ...
    DEV =
      this.app.plugins.enabledPlugins.has('hot-reload') &&
      this.app.plugins.plugins['hot-reload'].enabledPlugins.has(
        this.manifest.id,
      );
    if (DEV) {
      this.addCommand({
  ...
  async openVSCode() {
    var _a;
    if (
      !(this.app.vault.adapter instanceof import_obsidian2.FileSystemAdapter)
    ) {
      return;
    }
  ...
  async openVSCodeUrl() {
    ...
    if (openFile) {
      url += `/${filePath}`;
      const workspacePath = replaceAll(
